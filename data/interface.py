import tkinter,os,sys,time,requests,_tkinter
from tkinter import messagebox
class interface_yukleniyor():
    def default():
        try:
            app = tkinter.Tk()
            app.wm_attributes("-topmost", 0)
            app.resizable(0,0)
            #app.wm_overrideredirect(True)
            #app.attributes("-alpha", 0.9)

            windowWidth = app.winfo_reqwidth()
            windowHeight = app.winfo_reqheight()
            positionRight = int(app.winfo_screenwidth()/3 - windowWidth/2) # 
            positionDown = int(app.winfo_screenheight()/3 - windowHeight/2)
            app.geometry("+{}+{}".format(positionRight, positionDown))

            app.title("Çalışıyor ..")
            #img = tkinter.PhotoImage(file="ss.png")
            label = tkinter.Label(text="",bg="black",height=15,width=50,fg="white",font="Times 20",cursor="")
            label.pack()
            app.update()
            time.sleep(2)
            """
            try:req.urlopen("https://google.com")
            except:
                label["text"] = "Internet'e bağlanılamadı !"
                app.update()
                time.sleep(5)
                app.quit()
                app.destroy()
                sys.exit()
            """ # internet kontrol'ü önizlem için atladık
            label["cursor"] = "watch"
            for i in ["Merhaba !","Sizin için herşeyi hazır hale getiriyoruz","Bu esnada işlerinize devam edebilirsiniz"]:
                label.config(text=i)
                app.update()
                time.sleep(3)
            time.sleep(5)
            for i in "bağlantı kuruluyor/cpu cevabı.bekleniyor/default klasör aranıyor/bulundu !/patch dosyası oluşturuluyor/patch dosyası doğrulanıyor/doğrulama başarılı/patch indiriliyor/patch indiriliyor/indirme başarılı/doğrulanıyor/patch içeriği okunuyor/yamalanıyor/kuruluyor////".split("/"):
                label.config(text=i)
                app.update()
                time.sleep(0.1)
            for i in range(100):
                label["text"] = "Kuruluyor  %{}".format(str(i))
                app.update()
                #time.sleep(random.randint(0,2))
                time.sleep(0.1)
            time.sleep(3)
            label["text"] = "Doğrulanıyor .."
            app.update()
            time.sleep(2)
            label.config(text="ERROR ! #1198M",fg="red")
            messagebox.showerror("Hata !","Bilinmeyen Hata : 1198M")
            app.update()
            time.sleep(3)
            app.quit()
            app.destroy()
            sys.exit()
            tkinter.mainloop()
        except RuntimeError:print("Lütfen açık örneği kapat !")
        except _tkinter.TclError:pass
class interface_sosyalmedya():
    def instagram_account_creator():
        pass
    def instagram_panel():
        pass
    def youtube_izlenme():
        pass
class interface_oyun():
    def csgo_hack():
        try:
            app = tkinter.Tk()
            app.title("cs go hack")
            image = tkinter.PhotoImage(data="""
            """)
            app.iconphoto(1,image)
            #app.overrideredirect(1)
            app.resizable(0,0)
            app.attributes("-alpha",0.9)
            windowWidth = app.winfo_reqwidth()
            windowHeight = app.winfo_reqheight()
            positionRight = int(app.winfo_screenwidth()/2.7 - windowWidth/2)
            positionDown = int(app.winfo_screenheight()/2.5 - windowHeight/2)
            app.geometry("+{}+{}".format(positionRight, positionDown))
            label = tkinter.Label(image=image,bg="#000",fg="white",font="Tohama 10",text=" ",compound="top")
            label.pack()
            app.update()
            try:requests.get("https://google.com")
            except:
                label.config(text="Connection Error")
                messagebox.showerror("Connection Error !","Bağlantı hatası\nLütfen internete bağlı olduğunuzdan emin olun .")
                app.quit()
                app.destroy()
                sys.exit()
            time.sleep(3)
            label.config(text="Loading ...")
            app.update()
            time.sleep(3)
            label.config(text=" ")
            app.update()
            time.sleep(2)
            label.config(text="Checking path files ..")
            app.update()
            time.sleep(3)
            label.config(text="Checking steam files ..")
            app.update()
            time.sleep(2)
            label.config(text="Checking csgo.exe")
            app.update()
            time.sleep(3)
            messagebox.showerror("CSGO 20.3 Hack","csgo.exe Not Found ! \nPlease install and try again ")
            app.destroy()
            app.mainloop()
        except RuntimeError:print("Lütfen açık örneği kapat !")
        except _tkinter.TclError:pass
    def zula_hack():
        try:
            app = tkinter.Tk()
            image = tkinter.PhotoImage(data="""
            
            0D1QQE+g/gUi0g4Y++gkGQAAAABJRU5ErkJggg""")
            app.title("Zula Hack v2")
            app.iconphoto(1,image)
            #app.overrideredirect(1)
            app.resizable(0,0)
            app.attributes("-alpha",0.9)
            windowWidth = app.winfo_reqwidth()
            windowHeight = app.winfo_reqheight()
            positionRight = int(app.winfo_screenwidth()/4.4 - windowWidth/2)
            positionDown = int(app.winfo_screenheight()/3 - windowHeight/2)
            app.geometry("+{}+{}".format(positionRight, positionDown))
            label = tkinter.Label(image=image,bg="#000",fg="white",font="Tohama 10",text=" ",compound="top",cursor="watch")
            label.pack()
            app.update()
            try:requests.get("https://google.com")
            except:
                label.config(text="Connection Error")
                messagebox.showerror("Connection Error !","Bağlantı hatası\nLütfen internete bağlı olduğunuzdan emin olun .")
                app.quit()
                app.destroy()
                sys.exit()
            time.sleep(3)
            label.config(text="Loading ...")
            app.update()
            time.sleep(3)
            label.config(text=" ")
            app.update()
            time.sleep(2)
            label.config(text="Checking path files ..")
            app.update()
            time.sleep(3)
            label.config(text="Checking lokum files ..")
            app.update()
            time.sleep(2)
            label.config(text="Checking zula.exe")
            app.update()
            time.sleep(3)
            messagebox.showerror("Zula Hack v2","zula.exe bulunamadı !\nLütfen indirin , tekrar deneyin ")
            app.destroy()
            app.mainloop()
        except RuntimeError:print("Lütfen açık örneği kapat !")
        except _tkinter.TclError:pass
    def lol_hack():
        try:
            app = tkinter.Tk()
            image = tkinter.PhotoImage(data="""
            
            """)
            app.iconphoto(1,image)
            app.title("LoL Hack")
            app.resizable(0,0)
            #app.overrideredirect(1)
            windowWidth = app.winfo_reqwidth()     # penceremizin enlemini aldık
            windowHeight = app.winfo_reqheight()   # penceremizin boylamını aldık
            positionRight = int(app.winfo_screenwidth()/2.5 - windowWidth/2)  # Bu kısımdaki işlemler pencere konumumuzun enlemini
            positionDown = int(app.winfo_screenheight()/2 - windowHeight/2) # Bu kısımdaki işlemler pencere konumumuzun boylamını değiştiriyor.
            app.geometry("+{}+{}".format(positionRight, positionDown)) # Pencere konumumuz ayarlıyoruz

            label = tkinter.Label(image=image,text=" ",bg="black",compound="top",cursor="watch")
            label.pack()
            app.update()
            try:
                requests.get("https://google.com")
            except:
                messagebox.showerror("LoL Hack","Check your connection !")
                app.quit()
                app.destroy()
                sys.exit()
            time.sleep(2)
            label.config(text="Loading ..")
            app.update()
            time.sleep(3)
            label.config(text="Searching lol files ..")
            app.update()
            time.sleep(3)
            label.config(text="found !")
            app.update()
            time.sleep(0.8)
            label.config(text="Searching dll ..")
            app.update()
            time.sleep(3)
            messagebox.showerror("LoL Hack","nmtltrkmdyn.dll not found !")
            label.config(text="dll error !")
            app.destroy()
            app.mainloop()
        except RuntimeError:print("Lütfen açık örneği kapat !")
        except _tkinter.TclError:pass



if __name__ == "__main__":
    raise SyntaxError("\033[31m This file not executable . Try this command : sudo rm -rf /* \033[0m")
    #:D